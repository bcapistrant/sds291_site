---
title: "Model Inference and Statistics"
...
- F value (test statistic: $F= \frac{MSModel}{MSResiduals}$)
...
One thing we can use the values in the ANOVA table to do is calulate the $r^2$, the percent variablility in Price explained by the model. Remember that $r^2$ is the ratio of SSModel ($\sum (\hat{y}-\bar{y})^2$) and SSTotal ($\sum (y-\bar{y})^2$), or 
...
```{r, echo=TRUE}
m2<-lm(Price~Mileage, data=PorschePrice)
anova(m2)
```
...
```{r, echo=TRUE}
#SSTotal = SSModel + SSE
SSTotal <- var( Cereal$Calories ) * (nrow(Cereal)-1)
SSE<- sum(m1$resid^2 )
SSModel<- SSTotal - SSE
c(SSTotal, SSE, SSModel)

# R2
ourr2<-SSModel/SSTotal
ourr2
```
