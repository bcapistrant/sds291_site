...
intercepts <- c(coef(m2)["(Intercept)"],
                coef(m2)["(Intercept)"] + coef(m2)["DistGroupFarther Away"])

#putting these into a new data frame with the slope and transmission indicator variable
lines.df <- data.frame(intercepts = intercepts,
                       slopes = rep(coef(m2)["SquareFeet"], 2),
                       distgroup = as.factor(levels(RailsTrails$DistGroup)))
#making the plot
qplot(x = SquareFeet, y = Price2014, color = DistGroup, data = RailsTrails) + 
  geom_abline(aes(intercept = intercepts, 
                  slope = slopes, 
                  color = distgroup), data = lines.df)
```
