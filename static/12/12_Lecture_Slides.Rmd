...
Remember the Life Expectancy example we had for simple linear regression?
...
p2 <- densityplot(~year, data=bootstrap)
ladd(panel.abline(v=coef(fm)["year"], col="red", lwd=3), plot=p2)
```
...
coef(fm)["year"] + zs * sd(~year, data=bootstrap)
```
...
```{r, echo=TRUE}
qdata(~year, p=c(0.025, 0.975), data=bootstrap)
```
...
```{r, echo=TRUE}
qs <- qdata(~year, p = c(0.025, 0.975), data=bootstrap)$quantile
coef(fm)["year"] - (qs - coef(fm)["year"])
```
...
bootstrap <- do(10000) * coef(lm(Price~Mileage, data=resample(PorschePrice)))
