...
The data in the `Whickham` data set contains observations about women, and whether they were alive 20 years after their initial observation (`outcome` is a 2 level factor variable - Alive/Dead). You can learn more about these data from the `mosaicData` help feature if you'd like.

Our goal is to determine the association between `age` (in years) and likelihood of mortality within the next 20 years.

We will also work with the other explanatory variable: whether the woman was a `smoker` (binary: yes/no).

Bring in the relevant packages (above) and the data (below, from the mosaic package).
